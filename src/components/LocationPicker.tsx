    ...
    <div className="space-y-3">
      ...
      <div ref={wrapperRef} className="relative">
        ...
        {showResults && results.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-brand-surface border border-white/10 rounded-xl shadow-elevated z-30 max-h-60 overflow-y-auto animate-slide-down">
            {results.map((r, i) => (
              <button
                key={i}
                type="button"
                onClick={() => selectResult(r)}
                className="w-full text-left px-4 py-3 hover:bg-white/[0.04] transition-colors border-b border-white/[0.04] last:border-0 flex items-start gap-3"
              >
        ...
        {showResults && query.length >= 3 && !searching && results.length === 0 && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-brand-surface border border-white/10 rounded-xl shadow-elevated z-30 px-4 py-3">
