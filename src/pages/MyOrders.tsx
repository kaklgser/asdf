...
import { Package, ChefHat, Truck, CheckCircle, XCircle, Clock, Bell, ChevronRight } from 'lucide-react';
    ...
    return (
      <div className="min-h-[60vh] max-w-lg mx-auto px-4 py-8 bg-brand-bg">
        <div className="animate-pulse space-y-3">
          <div className="h-7 bg-white/[0.06] rounded-lg w-32 mb-6" />
          {[1, 2, 3].map((i) => (
    ...
    <Link
      ...
      className={`block rounded-xl p-3.5 transition-all active:scale-[0.98] ${
        isReady
          ...
          : 'bg-brand-surface border border-white/[0.06]'
      }`}
    >
      {isReady && (
...
function PastOrderCard({ order }: { order: Order }) {
  ...
  return (
    <Link
      to={`/track/${order.order_id}`}
      className="flex items-center justify-between bg-brand-surface rounded-xl px-3.5 py-3 border border-white/[0.06] transition-all active:scale-[0.98]"
    >
      <div className="min-w-0">
        <div className="flex items-center gap-2">
