...
function PrepCountdown({ confirmedAt, estimatedMinutes }: { confirmedAt: string; estimatedMinutes: number }) {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-brand-surface p-6">
            ...
            {showReadyBanner && isReadyForPickup && (
              <div className="relative overflow-hidden rounded-2xl bg-emerald-500 p-6 text-center text-white shadow-elevated animate-scale-in backdrop-blur">
                ...
                <div className="relative">
                  ...
                  <h2 className="mb-2 text-2xl font-black">Your Order is Ready!</h2>
                  <p className="mb-4 text-[14px] text-emerald-100">
                    Head to the counter to pick up order {order.order_id}
                  </p>
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-brand-surface p-6 transition-shadow duration-300">
              <div className="mb-5 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  >
                    {isReadyForPickup ? 'Ready for Pickup' : order.status.replace('_', ' ')}
                  </span>
                  <p className="mt-1 text-[12px] font-semibold capitalize text-brand-text-dim">{order.order_type}</p>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-brand-gold/10 p-6 backdrop-blur-sm">
                <p className="text-[14px] leading-relaxed text-brand-text-muted">
                  {order.order_type === 'delivery'
                    ...
                    : 'We are preparing your order. You will be notified when it is ready for pickup.'}
                </p>
              </div>
            )}
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-brand-surface p-6 transition-shadow duration-300">
              ...
              <div className="grid grid-cols-2 gap-3">
                <a
                  ...
                  className="flex items-center justify-center gap-2 rounded-xl border border-white/[0.06] py-3 text-[14px] font-semibold text-brand-text-muted transition-all duration-200 hover:border-brand-gold hover:text-brand-gold"
                >
