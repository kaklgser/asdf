...
export default function AdminZones() {
  ...
  async function loadZones() {
    const { data } = await supabase.from('delivery_zones').select('*').order('area_name');
    setZones(data || []);
    setLoading(false);
  }

  async function saveZone() {
    if (!editing || !editing.pincode.trim()) return;
    const payload = {
  ...
  return (
    <div>
      ...
      {zones.length === 0 ? (
        <div className="bg-brand-surface rounded-xl border border-white/[0.06] p-10 text-center text-brand-text-muted">No delivery zones</div>
      ) : (
        <div className="bg-brand-surface rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {zones.map((zone) => (
                  <tr key={zone.id} className="hover:bg-white/[0.04] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button
                          ...
                          className="p-1.5 hover:bg-white/[0.06] rounded text-brand-text-dim hover:text-white"
                        >
                          <Save size={14} />
                        </button>
