...
import { Phone, ChefHat, ArrowLeft, Loader2, ArrowRight } from 'lucide-react';
...
export default function ChefLogin() {
  ...
  async function handleSendOtp(e?: React.FormEvent) {
    ...
    if (digits === CHEF_NUMBER || digits === ADMIN_NUMBER) {
  ...
  async function handleVerifyOtp(token?: string) {
    ...
    if (!profileData || (profileData.role !== 'chef' && profileData.role !== 'admin')) {
            ...
            <form onSubmit={handleSendOtp} className="bg-brand-surface rounded-2xl p-6 border border-white/[0.06] space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="tel"
                    placeholder="Enter chef phone number"
                    value={phone}
              ...
              <div className="flex justify-center gap-2.5">
                {otp.map((digit, i) => (
                  <input
                    ...
                    className="w-11 h-13 text-center text-xl font-bold rounded-xl bg-white/[0.06] border border-white/[0.1] text-white focus:border-orange-400 focus:ring-1 focus:ring-orange-400/30 outline-none transition-all"
                  />
                ))}
              </div>
              ...
              <div className="text-center">
                {resendTimer > 0 ? (
                  <p className="text-brand-text-dim text-[13px]">
                    Resend in <span className="text-orange-400 font-semibold">{resendTimer}s</span>
                  </p>
                ) : (
                  <button
                    onClick={() => handleSendOtp()}
                    disabled={loading}
                    className="text-orange-400 text-[14px] font-semibold hover:underline underline-offset-2"
                  >
                    Resend OTP
                  </button>
                )}
